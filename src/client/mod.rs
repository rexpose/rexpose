...
impl Client {
    pub fn new(server_address: &str, mgmt_port: &u16, certificate_path: &Option<String>) -> Client {
        return Self { mgmt_port: *mgmt_port, server_address: server_address.to_string(), certificate_path: certificate_path.clone() }
    }
    ...
    async fn connect_internal(self) -> Result<ConnectedClient, Box<dyn Error>> {
        ...
        let tls_stream = timeout(TLS_CONNECTION_TIMEOUT, tls_connector.connect(server_name, mgmt_stream)).await??;
        log::debug!("TLS connection established");
        return Ok(ConnectedClient { client: self, tls_connector: tls_connector, mgmt_stream: tls_stream })
    }
}